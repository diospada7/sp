...
echo "           Created By Imam Mustofa    	\n";
echo "===============================================\n";
echo " [+] Nama pembuat : ";
$pembuat = trim(fgets(STDIN));
echo "[+] Email : ";
$mail = trim(fgets(STDIN));
$pass = "bebek123";
echo "[+} Simpan file hasil pembuatan account : ";
$file = trim(fgets(STDIN));
echo "[+] Jumlah : ";
$jmlh = trim(fgets(STDIN));
echo " Hai $pembuat, Hasil pembuatan account disimpan sebagai $file.txt
...
if ($json->status == "20") {
	...
	if ($json->status == 1) {
		...
		echo "[-] Email : $mail$i$nvl Pass : $pass => success \n";
		fwrite(fopen("$file.txt", "a"), "$email|$passedit\n");
	}	
}
}
